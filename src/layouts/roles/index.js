...
export default function Roles() {
  ...
  async function createNewRole() {
    ...
    try {
      ...
      await post("/roles/createRoles", reqBody);
  ...
  async function handleRowEditCommit(params) {
    ...
    if (field === "role_name") {
      setLoading(true);
      try {
        const reqBody = { id, role_name: value, created_by: userId };
        await post("/roles/updateRole", reqBody);
  ...
  async function handleDeleteRole(id) {
    setLoading(true);
    try {
      await post("/roles/deleteRole", { id });
    ...
    },
    {
      ...
      getActions: (params) => [
        <GridActionsCellItem
          ...
          label="Delete"
          onClick={() => handleDeleteRole(params.id)}
        />,
        <GridActionsCellItem
                  icon={<Eye color="blue" />}
                  label="View Logs"
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        >
          <MDTypography variant="body2" color="textSecondary">
            Users &gt; <span style={{ fontWeight: "bold", color: "#111827" }}>Role Creation</span>
          </MDTypography>
