...
export default function Entity() {
  ...
  const [entity_code, setEntityCode] = useState("");
  ...
  async function createNewEntity() {
    if (!name ||!entity_code) return;
    setLoading(true);
    try {
      const reqBody = { name: name,entity_code:entity_code, created_by: userId };
      await post("/entity/createEntity", reqBody);
  ...
  async function handleRowEditCommit(params) {
    const { id, field, value } = params; // destructure the cell edit

    // Find the current row from state
    const currentRow = entity.find(e => e.id === id);
    ...
    const reqBody = {
      ...
      entity_code: field === "entity_code" ? value : currentRow.entity_code,
      // add more fields here if needed
    };

    setLoading(true);
    try {
      await post("/entity/updateEntity", reqBody);
  ...
  async function handleDeleteEntity(id) {
    setLoading(true);
    try {
      await post("/entity/deleteEntity", { id });
    ...
    },
    {
      ...
      getActions: (params) => [
        <GridActionsCellItem
          ...
          label="Delete"
          onClick={() => handleDeleteEntity(params.id)}
        />,
        <GridActionsCellItem
          icon={<Eye color="blue" />}
          label="View Logs"
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        >
          <MDTypography variant="body2" color="textSecondary">
            Users &gt; <span style={{ fontWeight: "bold", color: "#111827" }}>Entity</span>
          </MDTypography>
